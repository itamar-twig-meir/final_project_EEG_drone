...
# the standard model
def build_eeg_drone_model_1(input_shape=(4, 64, 14), num_classes=6):
    ...
    model.add(TimeDistributed(MaxPooling1D(pool_size=2), name='td_max_pool_2'))
    model.add(TimeDistributed(Dropout(0.4), name='td_dropout_2'))
    model.add(TimeDistributed(Flatten(), name='td_flatten'))

    model.add(Bidirectional(LSTM(units=32, return_sequences=False, dropout=0.35, recurrent_dropout=0.4),
...
def build_eeg_drone_model_2(input_shape=(4, 64, 14), num_classes=6):
    ...
    model.add(TimeDistributed(Conv1D(filters=128, kernel_size=1, padding='valid'), name='spatial_filter'))
    model.add(BatchNormalization(name = "batch_normalization_1"))
    ...
    model.add(TimeDistributed(Conv1D(filters=512, kernel_size=4, padding='same'), name='td_conv_2'))
...
# compact model
def build_eeg_drone_model_3(input_shape=(4, 64, 14), num_classes=6):
    inputs = tf.keras.Input(shape=input_shape)

    # treat each time window independently first
    x = TimeDistributed(Conv1D(filters=64, kernel_size=16, padding='same', use_bias=False))(inputs)
    x = BatchNormalization()(x)

    # depthwise conv — learns spatial filter per channel
    x = TimeDistributed(Conv1D(filters=128, kernel_size=1, padding='valid',
                               use_bias=True, kernel_regularizer=l2(0.01)))(x)
    x = BatchNormalization()(x)
    ...
    x = TimeDistributed(Conv1D(filters=192, kernel_size=8, padding='same',
                               use_bias=False))(x)
    x = BatchNormalization()(x)
    x = TimeDistributed(Activation('elu'))(x)
...
def build_eeg_drone_model_4(input_shape=(256, 14), num_classes=6):
    ...
    model.add(Bidirectional(LSTM(units=128, return_sequences=True, dropout=0.3)))
    model.add(BatchNormalization())
    model.add(Bidirectional(LSTM(units=64, return_sequences=False, dropout=0.3)))
