...
def train_on_moabb(model):
    ...
    min_class_size = min(np.bincount(y_train_labels))
    print(f"Balancing all classes to exactly {min_class_size} samples each.")

    balanced_indices = []
    for class_index in range(5):
        indices_for_this_class = np.where(y_train_labels == class_index)[0]
        chosen_indices = np.random.choice(indices_for_this_class, min_class_size, replace=False)
   ...
   y_train_integers = np.argmax(Y_train, axis=1)
    ...
    train_loss, train_acc = model.evaluate(X_train_balanced, Y_train_balanced, verbose=0)
...
def test_on_unseen_subjects(model):
    ...
    plot_confusion_matrix(Y_test, y_pred_unseen, "Confusion Matrix: Unseen Humans (Subjects 100-109)")
